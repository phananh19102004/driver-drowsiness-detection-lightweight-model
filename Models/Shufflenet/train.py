...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            progress_bar = tqdm(
                dataloaders[phase],
                desc=f'{phase.upper()} [{epoch+1}/{num_epochs}]',
                leave=False
            )
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('ShuffleNetV2 Training Loss')
    plt.legend()
    plt.grid(True)
    plt.show()
...
if __name__ == "__main__":
    ...
    torch.save(
        model_ft.state_dict(),
        'drowsy_driver_shufflenetv2.pth'
    )
    print("Saved model: drowsy_driver_shufflenetv2.pth")